...
<%@ page language="java" contentType="text/html; charset=UTF-8"
    ...
    add계열의 메서드를 통해 응답헤더를 추가한다.
    	addFateHeader(문자열, long형의 타임스템프)
    	 : 응답헤더 중 날짜를 추가하는 메소드로, 세계 표준시를 사용해서 설정한다.
    	   대한민국은 표준시 +09, 즉 9시간이 느리므로 09시로 설정해야 정상적인 날짜가 출력된다.
    	   만약 9시 이전의 시간이 설정되면 어제날짜가 나온다.
    */
    // 응답헤더에 값 추가
    ...
    response.addHeader("myName", add_str); // 문자형식 : 홍길동
    /*
    set계열의 메서드는 기존의 응답레더값을 수정한다.
    */
    response.setHeader("myName", "안중근"); // 앞에서 설정한 값을 '안중근'으로 수정한다.
    ...
    */
    response.setHeader("Pragma", "no-cache"); // HTTP/1.0에서 사용했던 걸정(호환성때문에 남아있음)
...
<body>
	...
	Collection<String> headerNamse = response.getHeaderNames();
	// 개선된(확장) for문을 통해 얻어온 갯수만큼 반복한다.
	for(String hName : headerNamse){
		// 응답헤더값을 얻어온다.
	...
	/*
		첫번째 출력결과에서는 myNumber라는 헤더명이 2번 출력되었지만 동일한 값이 출력된다.
	...
	*/
	...
	<%
	/*
		myNumber라는 헤더명으로 2개의 값을 추가했음으로 아래에서는
		각각의 값이 정상적으로 출력된다.
		즉, add 계열의 메서드는 헤더명을 통일하게 사용하더라도 헤더값은 추가된다.
	*/
