...
$(function(){
	$("#btnLogin2").click(function(){
		...
		/*
			jQuery의 Ajax관련 메서드의 파라미터는 JSON형태로 조립한후 서버로 전송해야한다.
			이때 <form>태그의 하위의 모든 폼값을 한꺼번에 JSON으로 만들고 싶다면
			serialize()메서드를 사용하면 된다.
		*/
		...
		$.post(
			...
			function(resData){
				...
				$.post() 메서드는 형식을 지정할 수 없어 무조건 text형식으로만 콜백받을 수 있다.
				
				따라서 JSON데이터를 콜백 받았다면
				파싱을 위해 JSON.parse()메서드를 한번 실행한 후 파싱해야 한다.
				*/
