...
<body>
	...
	<!-- 
		if(number % 2 == ) 과 동일한 조건의 if문으로
		해당 조건의 결과가 result에 저장된다.
	 -->
	<c:if test='${number mod 2 eq 0 }' var="result">
		${ number }는 짝수입니다.<br> <!-- 결과가 true임으로 출력됨 -->
	</c:if>
	result : ${ result }<br><!-- true가 출력됨 -->
	
	<!-- 
		JSTL의 if태그는 else구문이 따로없음으로 두개의 if태그를 통해
		else와 같이 사용해야 한다.
	...
	<c:if test="${ not result2 }"><!-- 앞의 result2의 not조건을 통해 else를 구현 -->
		'Java'가 아닙니다.<br>
	</c:if>
	...
	<br>
	<hr>
	<h4>연습문제] if태그(단 EL과 JSTL의 if태그만을 이용해서 구현하시오.)</h4>	
	<!-- 
	...
	<!-- 내가 테스트 -->
	<c:set var ="userId" value="${ param.user }"/>
	<c:set var ="userPw" value="${ param.pass }"/>
	
	<c:if test="${ not empty userId and not empty userPw }" var="LoginFormempty">
		<c:if test="${ userId eq 'kosmo' }" var="LoginId">
			<c:if test="${ userPw eq '1234' }" var="LoginPw">
				"kosmo님 하이룽~"
			</c:if>
			
			<c:if test="${ not LoginPw }" >
				"비번을 확인하세요"
			</c:if>
		</c:if>
		
		<c:if test="${ not LoginId }" >
			"아이디를 확인하세요"
		</c:if>
	</c:if>
	
	<c:if test="${not LoginFormempty }">
		"아이디와 패스워드를 입력해주세요"
	</c:if>
