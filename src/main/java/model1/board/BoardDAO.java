...
import common.JDBConnect; // DB연결을 위한 클래스로 패키지가 다름으로 import해야한다.
...
public class BoardDAO extends JDBConnect {
	...
	public int selectCount(Map<String, Object> map) {
		...
		try {
			// 정적쿼리문 실행을 위한 Statement객체 생성 (정적쿼리문이란: ?가 없는 쿼리문)
	...
	public List<BoardDTO> selectList(Map<String, Object> map){
		/*
		 ...
		 여러가지의 List컬렉션 중 동기화가 보장되는 Vector을 사용한다.
		 */
		List<BoardDTO> bbs = new Vector<BoardDTO>();
		/*
		 목록에 출력할 게시물을 추출하기 위한 쿼리문으로 항상 일련번호의 역순(내림차순)으로 정렬해야 한다.
		 게시판의 목록은 최근 게시물이 제일 앞에 노출되기 때문이다.
		 */
	...
	public void updateVisitCount(String num) {
		// visitcount 컬럼은 number 타입임으로 덧셈이 가능하다.
	...
	public List<BoardDTO> selectListPage(Map<String, Object> map) {
		...
		// 검색 조건 추가(검색어가 있는 경우에만 where절을 추가함)
		if(map.get("searchWord") != null) {
			query += " WHERE "+ map.get("searchField")
				+ " LIKE '%" + map.get("searchWord") + "%' ";
		}
		...
		try {
			...
			psmt = con.prepareStatement(query);
			// 일파라미터 설정 : 구간을 위한 start, end를 설정함
